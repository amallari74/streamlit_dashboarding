...
def fetch_historical_invoice_totals_by_vendor(invoice_month = None, schema: str = "", database: str = None):
    """Fetch historical invoice totals by vendor"""
    if invoice_month is None:
        invoice_month = date.today().replace(day=1)
    else: 
        invoice_month = f'{invoice_month}'
        
    query = f"""
    select 
        acptms.invoice_date,
        acptms.vendor as vendor_name,
        acptms.business_unit_code,
        max(acptms.gross_revenue_usd) as gross_revenue_usd,
        sum(acptms.gross_revenue_usd) as revenue_billed_usd
    from 
        mart_observability_and_automation.agg_cira_product_type_month_snapshot acptms
    where acptms.dbt_valid_to is null
        and acptms.invoice_date = '{invoice_month}' 
    group by
        vendor_name,
        acptms.invoice_date,
        acptms.business_unit_code
    having 
        sum(acptms.gross_revenue_usd) >= 0 
        and max(acptms.gross_revenue_usd) >= 0
    order by 
        revenue_billed_usd desc 
    """
    return db_util.query(query, params={"invoice_month": invoice_month})