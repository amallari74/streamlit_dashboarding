...
+                                              vendor_usage_variance['usd_prior_month_revenue'].fillna(0))
        ...
        # Ensure revenue columns are float
        final_df['Current Month Revenue (USD)'] = final_df['Current Month Revenue (USD)'].astype(float)
        final_df['Prior Month Revenue (USD)'] = final_df['Prior Month Revenue (USD)'].astype(float)
        final_df['Revenue Difference (USD)'] = final_df['Revenue Difference (USD)'].astype(float)
        ...
        st.dataframe(
            ...
            column_config={
                "Current Month Revenue (USD)": st.column_config.NumberColumn(
                    format="$%.2f",
                    help="Current month revenue"
                ),
                "Prior Month Revenue (USD)": st.column_config.NumberColumn(
                    format="$%.2f",
                    help="Prior month revenue"
                ),
                "Revenue Difference (USD)": st.column_config.NumberColumn(
                    format="$%.2f",
                    help="Revenue difference"
                ),
