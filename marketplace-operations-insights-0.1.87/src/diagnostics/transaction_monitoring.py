...
from diagnostics.models.transaction_model import fetch_duplicate_cira_line_items, fetch_missing_subscriptions_invoice_line_items, fetch_missing_subscription_invoice_line_summary, fetch_cube_js_missing_subscriptions
...
def render_cube_js_missing_subscriptions_chart():
    cube_js_missing_subscriptions_chart = """
    digraph {
        rankdir=TB;
        ...
        { rank=same; B C}
        A [label="Missing Subscriptions (DBT)", fillcolor="#d4942f", fontcolor=white, width=2.0, height=1.0, style="filled"];
        B [label="billing_run_context__missing_subscription_detail (Cube.js -> Cube)", fillcolor="#a62ad4", fontcolor=white, width=2.0, height=1.0, style="filled"];
        C [label="billing_run_context__missing_subscription (Cube.js -> View)", fillcolor="#a62ad4", fontcolor=white, width=2.0, height=1.0, style="filled"];
        D [label="Streamlit", fillcolor="#6ebbeb", fontcolor=white, width=2.0, height=1.0, style="filled"];
    ...
    """
    return st.graphviz_chart(cube_js_missing_subscriptions_chart)
...
in addition to net values of the quantity and price changes.**
    ...
    missing_subscriptions, duplicates, missing_subscriptions_graph, cube_js_missing_subscriptions = st.tabs(["Missing Subscription Charges", "Duplicate Invoice Line Items","Subscription to Invoice Monitoring (Graphs)", "Cube.js Missing Subscriptions"])
    ...
    with cube_js_missing_subscriptions:
        with st.expander("**Cube.js Missing Subscriptions**"):
            st.markdown(f"""
            Cube-Js Missing Subscriptions for {formatted_date}. The data is pulled from the cube.js billing_run_context__missing_subscription view 
            and meant to improve loading performance into streamlit.
            
            ```
            Model Criteria (Cube.js View)
            - transaction_month: {selected_date}
            - omt_subscription_match_status: False
            - omt_prorate_match_status: False

            Data Process: 
            1.Data is built via DBT and stored in Redshift as a table
            - Table: mart_observability_and_automation.fact_subscription_monthly_billing_projection_combined
            - Timing: Airflow Process run every hour from days 1-7 and every 4 hours rest of the month. 

            2.Missing Subscriptions are stored in a non-public cube called billing_run_context__missing_subscription_detail
            - Cube: billing_run_context__missing_subscription_detail
            - Timing: Refresh is processed every hour daily
            - Note: This cube is not public and not accessible via the SQL API

            3.A View is built on top of the cube to allow the data to be queried via the SQL API. 
            - View: billing_run_context__missing_subscription
            - Timing: Refresh is processed every hour daily (same as the cube)
            - Note: This view is public and accessible via the SQL API. Additional columns are removed that were deemed not nessesary. 
            - Columns: 
                - transaction_month: The month of the transaction
                - partner_id: The ID of the partner
                - company_id: The ID of the company
                - original_subscription_id: The ID of the original subscription
                - max_subscription_id_transaction_month: The month of the maximum subscription ID
                - min_subscription_id_transaction_month: The month of the minimum subscription ID
                - successor_completed_line_item_id: The ID of the successor completed line item
                - prior_completed_line_item_id: The ID of the prior completed line item
                - successor_subscription_start_date: The start date of the successor subscription
                - successor_subscription_end_date: The end date of the successor subscription
                - successor_billing_cycle_start_date: The start date of the successor billing cycle
                - successor_commitment_term_end_date: The end date of the successor commitment term
                - cancellation_date: The date of the cancellation
                - estimated_billing_renewal_date: The estimated billing renewal date
                - partner_buy_rate: The partner buy rate
                - actual_retail_price: The actual retail price
                - wholesale_buy_rate: The wholesale buy rate
                - missed_revenue_non_usd: The missed revenue non USD
                - missed_revenue_usd: The missed revenue USD
                - cira_prorate_total: The total prorate charges
                - cira_prorate_count: The count of prorate charges
                - net_subscription_and_prorate_non_usd: The net subscription and prorate non USD
                - net_subscription_and_prorate_usd: The net subscription and prorate USD
                - ledger_charge_count: The count of ledger charges
                - service_charge_amount: The service charge amount
                - service_charge_amount_usd: The service charge amount USD
                - estimated_billing_start_period: The start period of the estimated billing
                - estimated_billing_end_period: The end period of the estimated billing
                - product_unit_of_measure: The unit of measure of the product
                - successor_rate_plan_type: The rate plan type of the successor
                - charge_type: The charge type
                - completed_order_purchase_order_number: The purchase order number of the completed order
                - currency_guid: The GUID of the currency
                - completed_line_item_guid: The GUID of the completed line item
                - subscription_guid: The GUID of the subscription
            ```
            """)
            render_cube_js_missing_subscriptions_chart()
        try: 
            start_time = time.perf_counter()
            cube_js_missing_subscriptions_df = fetch_cube_js_missing_subscriptions(selected_date)
            end_time = time.perf_counter()
            elapsed_time = end_time - start_time
            record_count = len(cube_js_missing_subscriptions_df)
            
            performance_time, record_count_col = st.columns(2)
            with performance_time:
                st.metric(
                    label="Performance Time",
                    value=f"{elapsed_time:.2f}s",
                    delta=None,
                    help="Time taken to fetch and process data"
                )
            with record_count_col:
                st.metric(
                    label="Record Count",
                    value=f"{record_count:,}",
                    delta=None,
                    help="Total number of records returned"
                )
            
            st.dataframe(cube_js_missing_subscriptions_df, 
                use_container_width=True,
                hide_index=True,
                column_config={
                    "transaction_month": st.column_config.DateColumn(format="YYYY-MM-DD"),
                    "partner_id": st.column_config.NumberColumn(format="%d"),
                    "company_id": st.column_config.NumberColumn(format="%d"),
                    "original_subscription_id": st.column_config.NumberColumn(format="%d"),
                    "max_subscription_id_transaction_month": st.column_config.NumberColumn(format="%d"),
                    ...
                    "successor_completed_line_item_id": st.column_config.NumberColumn(format="%d"),
                    "prior_completed_line_item_id": st.column_config.NumberColumn(format="%d"),
                    "successor_subscription_start_date": st.column_config.DateColumn(format="YYYY-MM-DD"),
                    "successor_subscription_end_date": st.column_config.DateColumn(format="YYYY-MM-DD"),
                    "successor_billing_cycle_start_date": st.column_config.DateColumn(format="YYYY-MM-DD"),
                    "successor_commitment_term_end_date": st.column_config.DateColumn(format="YYYY-MM-DD"),
                    "cancellation_date": st.column_config.DateColumn(format="YYYY-MM-DD"),
                    "estimated_billing_renewal_date": st.column_config.DateColumn(format="YYYY-MM-DD"),
                    "partner_buy_rate": st.column_config.NumberColumn(format="$%.2f"),
                    "actual_retail_price": st.column_config.NumberColumn(format="$%.2f"),
                    "wholesale_buy_rate": st.column_config.NumberColumn(format="$%.2f"),
                    ...
                    "net_subscription_and_prorate_non_usd": st.column_config.NumberColumn(format="$%.2f"),
                    "net_subscription_and_prorate_usd": st.column_config.NumberColumn(format="$%.2f"),
                    "ledger_charge_count": st.column_config.NumberColumn(format="%d"),
                    "service_charge_amount": st.column_config.NumberColumn(format="$%.2f"),
                    "service_charge_amount_usd": st.column_config.NumberColumn(format="$%.2f"),
                    "estimated_billing_start_period": st.column_config.DateColumn(format="YYYY-MM-DD"),
                    "estimated_billing_end_period": st.column_config.DateColumn(format="YYYY-MM-DD") })
            ...
            st.error(f"Error fetching cube.js missing subscriptions data: {str(e)}")
            cube_js_missing_subscriptions_df = pd.DataFrame()
